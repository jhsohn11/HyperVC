...
def train(args):
    ...
    if args.dataset == 'ICEWS14':
        ...
        valid_data, valid_times = utils.load_quadruples('./data/' + args.dataset, 'test00.txt')
    ...
    model_state_file_backup = 'models/' + args.dataset + '/gat_backup.pth'
    ...
    global_k = checkpoint_global['global_k']
    model.global_emb = global_emb
    model.global_k = global_k
    
    checkpoint_backup = torch.load(model_state_file_backup, map_location=lambda storage, loc: storage)
    model.load_state_dict(checkpoint_backup['state_dict'])
    ...
    while True:
        ...
        torch.save({'state_dict': model.state_dict(), 'epoch': epoch, 
                ...
                'latest_time': model.latest_time, 'global_emb': model.global_emb, 'global_k':model.global_k},
               model_state_file_backup)
...
#        if epoch % args.valid_every == 0 and epoch >= int(args.max_epochs/2):
        if epoch % args.valid_every == 0:
            ...
            print("time: {:.4f}".format(t5 - t4))
            if mrr > best_mrr:
                best_mrr = mrr
                torch.save({'state_dict': model.state_dict(), 'epoch': epoch, 'global_k':model.global_k,
                        's_hist': model.s_hist_test, 's_cache': model.s_his_cache,
                        'o_hist': model.o_hist_test, 'o_cache': model.o_his_cache,
                        's_hist_t': model.s_hist_test_t, 's_cache_t': model.s_his_cache_t,
                        'o_hist_t': model.o_hist_test_t, 'o_cache_t': model.o_his_cache_t,
