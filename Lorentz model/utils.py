...
def get_hierarchical_score(inPath, fileName, fileName2=None, fileName3=None):
    dic = {}
    with open(os.path.join(inPath, fileName), 'r') as fr:
        for line in fr:
            line_split = line.split()
            dic[int(line_split[0])] = float(line_split[1])
    if fileName2 is not None:
        with open(os.path.join(inPath, fileName2), 'r') as fr:
            for line in fr:
                line_split = line.split()
...
def get_sorted_s_r_embed_rgcn(s_hist_data, s, r, ent_embeds, graph_dict, global_emb):
    ...
    return s_len_non_zero, s_hist_t_sorted, s_tem, r_tem, batched_graph, node_ids_graph, global_emb_list
