...
class LookupEmbedding(Module):
    ...
    def __init__(
        self, num_embeddings, embedding_dim, k=torch.tensor([-1]), learnable=True, _weight=None
    ):
        ...
        self.manifold = geoopt.manifolds.Lorentz(k=-softplus(-k), learnable=learnable)
        
        if _weight is None:
            _weight = torch.randn(num_embeddings, *embedding_dim)
            self.weight = geoopt.ManifoldParameter(geoopt.manifold.lorentz.lmath.project(_weight, k=-softplus(-k)), manifold=self.manifold)
